...
int main() {
  ...
  PID pid;
  pid.Init(0.05, 0.003, 1);

  PID pid_speed;
  pid_speed.Init(0.3, 0.003, 0.001);
      ...
      if (length && length > 2 && data[0] == '4' && data[1] == '2') {
        ...
        if (s != "") {
          ...
          if (event == "telemetry") {
            ...
            double speed_current = std::stod(j[1]["speed"].get<std::string>());
//          double angle = std::stod(j[1]["steering_angle"].get<std::string>());
            double speed_target = 50;
            double cte_speed = 0.1 * (speed_target - speed_current);
            ...
            } else if (throttle_value < throttle_min) {
              throttle_value = throttle_min;
            }


            pid.UpdateError(-cte_steer);
